...
def request_code(email: str, db: Session) -> None:
...
def verify_code(email: str, code: str, db: Session) -> tuple[str, str, User]:
    user = db.query(User).filter(User.email == email, User.is_active.is_(True)).first()
    if not user:
...
def refresh(raw_refresh_token: str, db: Session) -> str:
    token_row = (
